---
title: "Rebuilding referral to pay for users, not signups"
company: "Fold"
date: "2025"
description: "Rebuilt Fold's referral program so bonuses paid out on real usage, not email addresses — and built an affiliate channel with the biggest names in bitcoin media. In peak months it drove ~20% of new growth."
impact:
  - "Rebuilt Fold's referral program end-to-end: placement, fraud gating, economics, and a usage-based reward mechanic — drove ~20% of new growth in peak months"
...
tldr: "Fold's referral program was a buried toggle that paid flat sats for signups, so it paid for email addresses, not users. I rebuilt it in three passes — findability, economics, then a usage-based reward — and extended the same logic into an affiliate channel with the biggest voices in bitcoin media."
status: "Shipped"
...
Fold's referral program was a toggle buried in your profile settings. It paid a flat chunk of sats when someone you invited made an account — so it paid for email addresses, not users. You could fake that. And the bonus was denominated in sats, so every time bitcoin's price climbed, our cost climbed with it.

I rebuilt it. By the end it worked completely differently. The bonus only paid out when a referred user actually used the product, not when they signed up. The biggest names in bitcoin media were sending us users through it. And in a good month, it drove around a fifth of our new growth.
...
I tested the three things that mattered: **the amount, the messaging, and the placement.**

First, the fraud gate. A signup bonus invites fake emails, so I gated it: the person you referred had to get a real Fold card and become active before either of you got paid. The reward followed real behavior, not an email address.
...
The messaging was clunky at first, so I cleaned it up alongside the placement work.

## Pass 3 — pay for active users, not signups
...
The point: your earnings tracked the ongoing value your referrals created, not the one-time act of signing up. It turned referrers into people invested in their invitees actually using the product.
...
- **The read on what worked.** I set up automation to track signups per affiliate over time, then looked at which episodes or email blasts actually moved the number — so I could see what content drove conversions, not just impressions.
...
- **I tracked signups and reasoned about payback, but I didn't retain clean channel-level dashboards or run a formal incrementality test.** I couldn't tell you, with a holdout, how many of those referred users we'd have acquired anyway. At Fold's scale and budget, volume and unit-cost discipline were the bar. At a larger scale, the first thing I'd add is real incrementality — geo or audience holdouts — so we're paying for net-new acquisition, not subsidizing growth we'd have gotten for free.
...
- **Attribution is a design decision, not a reporting afterthought.** Per-campaign QR codes and custom affiliate links cost almost nothing to build and told me where growth was really coming from.
